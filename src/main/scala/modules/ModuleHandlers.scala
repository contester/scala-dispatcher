...
class Win32ModuleFactory(api: InvokerAPI) extends ModuleFactory(api) {
  ...
  def generate: Future[Seq[ModuleHandler]] =
    ...
    add((api.disks / "Python36" / "Python.exe") ++ (api.disks / "Python35" / "Python.exe") ++ (api.disks / "Python34" / "Python.exe") ++ (api.disks / "Programs" / "Python-3" / "Python.exe"), new PythonModuleHandler("py3", _)) +
