...
import org.stingray.contester.problems.{ProblemBase, ProblemArchiveUploadException, SimpleProblemDb, Test}
...
class SolutionTester(invoker: InvokerSimpleApi) extends Logging {
  def apply(submit: SchedulingKey, sourceModule: Module, problem: ProblemBase, progress: SingleProgress,
...
class BinarySolution(invoker: InvokerSimpleApi, store: TestOutputStore,
                     submit: SchedulingKey, problem: ProblemBase,
